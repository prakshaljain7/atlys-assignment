...
function Posts() {
  ...
  return (
    <div className="flex justify-center h-[100vh]">
      <div className="w-1/2">
        <h1 className="text-light-gray font-medium mt-10">Hello Jane</h1>
        <p className="text-dark-gray text-[16px]">
          How are you doing today? Would you like to share something with the
          community 🤗
        </p>
        <CreatePost onClick={() => set_show_register_popup(true)} />
        {post_list.map((post, index) => {
          return (
            <PostEntry
              post={post}
              key={`post${index}`}
              onClick={() => set_show_register_popup(true)}
            />
          );
        })}
      </div>
      {show_login_popup && (
        <div className="fixed flex justify-center items-center h-[100vh] w-[100vw] backdrop-blur-lg">
          <LoginPopup
            on_register_click={() => {
